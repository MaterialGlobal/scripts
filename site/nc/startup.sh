...
function warn_java() {
    ...
    read -rp "Type 'yes' if it's set to Java or 'no' if it's not: " option
    case $option in
        yes) run_java ;;
        no) image_fail ;;
...
function warn_node() {
    ...
    echo "WARNING: Otherwise, your App Container will not run at all."
    read -rp "Type 'yes' if it's set to NodeJS or 'no' if it's not: " option
    case $option in
        yes) run_node ;;
        no) image_fail ;;
        *) echo "Invalid option. Exiting." ;;
...
function warn_python() {
    ...
    echo "WARNING: Otherwise, your App Container will not run at all."
    read -rp "Type 'yes' if it's set to Python or 'no' if it's not: " option
    case $option in
        yes) run_python ;;
        no) image_fail ;;
        *) echo "Invalid option. Exiting." ;;
...
function run_python() {
    if [[ -d .git ]]; then git pull; fi
    if [[ -n "${PY_PACKAGES}" ]]; then pip install -U --prefix .local "${PY_PACKAGES}"; fi
